...
import { TagsCriterion, TagsCriterionOption } from "src/models/list-filter/criteria/tags";
...
export const TagHero: React.FC<ITagHeroProps> = ({ tag }) => {
    ...
    // Fetch 5 random scenes for this tag
    const filter = useMemo(() => {
        ...
        tagCriterion.value = {
            items: [{ id: tag.id, label: tag.name || "" }],
            excluded: [],
            depth: 0,
        };
        tagCriterion.modifier = GQL.CriterionModifier.Includes;
        f.criteria.push(tagCriterion);

        return f;
    }, [configuration, tag.id, tag.name]);
