...
// screenshotBatchSize bounds how many marker screenshots one nativegen.Frames
// call decodes at once.
//
// nativegen.Frames does not return until every requested frame is decoded, so
// every frame it is given is held as uncompressed RGBA, at the source's full
// resolution, simultaneously, until the whole call finishes — nothing is
// released frame by frame. Unbounded, that peak is marker_count times a full
// frame at source resolution: on an 8K file with a few dozen markers that is
// tens of gigabytes held at once, for a batch whose entire point was to be
// cheap. Measured on a real run: Go's own heap stats (runtime.MemStats.Sys)
// showed a ~17.8GB high-water mark within under a minute of process start,
// with HeapAlloc low and falling and HeapReleased climbing throughout — proof
// the GC was never behind, only that something asked for far too much heap in
// one shot. That is this function.
//
// Chunking trades some of the run-sharing benefit at chunk boundaries — two
// markers that would have shared a keyframe's run-up now might not, if a
// chunk break falls between them — for a peak that cannot grow past
// screenshotBatchSize frames no matter how large the scene or the source. It
// also improves failure isolation over one call for the whole scene: a bad
// frame now fails only its own chunk, per the all-or-nothing trade
// nativegen.Frames still makes within a chunk, rather than every screenshot
// in the scene.
const screenshotBatchSize = 16

// generateNativeMarkerScreenshots decodes every wanted screenshot in a scene
// in fixed-size batches instead of one nativegen.Frames call per marker.
//
...
// one decoder stood up per marker into one per batch instead of one per
// marker.
//
...
// safe rather than merely fast when it works: nativegen.Frames fails all of
// one batch's requested times together, the same all-or-nothing trade Preview
// and Sprite already make for their own callers, just drawn at batch width
// here instead of one asset. When a batch fails, every marker in it falls
// through to being decoded — and, if that also fails, generated by ffmpeg —
// on its own, exactly as if this function had never run for that marker.
func (t *GenerateMarkersTask) generateNativeMarkerScreenshots(ctx context.Context, videoFile *models.VideoFile, scene *models.Scene, sceneMarkers []*models.SceneMarker, sceneHash string, native *nativeMarkerSession) map[int]bool {
	...
	var batches int
	for start := 0; start < len(jobs); start += screenshotBatchSize {
		end := min(start+screenshotBatchSize, len(jobs))
		batch := jobs[start:end]
		batches++

		times := make([]float64, len(batch))
		for i, j := range batch {
			times[i] = j.seconds
		}
		...
		if err != nil {
			if ctx.Err() == nil {
				logger.Infof("[generator] native marker screenshot batch declined %s (%d of %d markers), falling back per marker: %v",
					videoFile.Path, len(batch), len(jobs), err)
			}
			continue
		}
	...
	logger.Debugf("[generator] native (%s) produced %d/%d marker screenshots for %s in %d batch(es)",
		nativegen.Describe(), len(done), len(jobs), videoFile.Path, batches)
