...
/**
 * VR sprite tiles are generated at 320×180 (2× the standard 160×90).
 * This scale factor shrinks them to ~160×90 so they fit within
 * the 140px-tall scrubber alongside standard tiles.
 */
const VR_SPRITE_SCALE = 0.5;
// The sprite grid is always 9×9 (see sprite.go).
const SPRITE_COLS = 9;
const SPRITE_ROWS = 9;

const SCRUBBER_HEIGHT = 140;

interface IVRScenePlayerScrubberProps {
...
export const VRScenePlayerScrubber: React.FC<IVRScenePlayerScrubberProps> = ({
  file,
  scene,
  time,
  start = 0,
  ...
  useEffect(() => {
    ...
    spriteInfo.forEach((sprite) => {
      ...
      // Scale VR sprite dimensions to fit within the scrubber
      const scaledW = sprite.w * VR_SPRITE_SCALE;
      const scaledH = sprite.h * VR_SPRITE_SCALE;
      totalWidth += scaledW;
      const left = totalWidth - scaledW;

      // The sprite sheet is a 9×9 grid; scale background-size proportionally
      // so that background-position offsets match the scaled coordinates.
      const sheetW = sprite.w * SPRITE_COLS * VR_SPRITE_SCALE;
      const sheetH = sprite.h * SPRITE_ROWS * VR_SPRITE_SCALE;

      const style: CSSProperties = {
        width: `${scaledW}px`,
        height: `${scaledH}px`,
        backgroundPosition: `${-(sprite.x * VR_SPRITE_SCALE)}px ${-(sprite.y * VR_SPRITE_SCALE)}px`,
    ...
    logger.debug("[VRScrubber] Generated sprite items", { count: newSprites.length });
