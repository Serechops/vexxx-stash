...
const StudioPage: React.FC<IProps> = ({ studio, tabKey }) => {
  ...
  return (
    ...
    >
      ...
      <DetailBreadcrumbs
        crumbs={[
          { label: intl.formatMessage({ id: "studios" }), to: "/studios" },
          ...(studio.parent_studio
            ? [{ label: studio.parent_studio.name, to: `/studios/${studio.parent_studio.id}` }]
            : []),
          { label: studio.name ?? intl.formatMessage({ id: "studio" }) },
        ]}
      />
      ...
      <Box
        ...
        sx={{
          ...
          position: 'relative',
          width: '100%',
          ...(collapsed && { '& .logo': { maxHeight: '200px' } }),
          ...(!collapsed && !compactExpandedDetails && { maxWidth: '100%' }),
        }}
      >
        <BackgroundImage
          ...
          <Box className="flex flex-wrap">
            <Box className="studio-head flex-1">
              ...
              <Box sx={{
                ...
                display: 'inline-flex',
                mt: 0.5,
                '& .rating-stars-precision-full .star-rating-number': { minWidth: '0.75rem' },
                '& .rating-stars-precision-half .star-rating-number, & .rating-stars-precision-tenth .star-rating-number': { minWidth: '1.45rem' },
                '& .rating-stars-precision-quarter .star-rating-number': { minWidth: '2rem' },
              }}>
                <RatingSystem
