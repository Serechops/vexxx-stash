...
type SceneOHistoryLoaderConfig struct {
	...
	Fetch func(keys []int) ([][]Timestamp, []error)
...
// Load a Timestamp by key, batching and caching will be applied automatically
func (l *SceneOHistoryLoader) Load(key int) ([]Timestamp, error) {
	return l.LoadThunk(key)()
}

// LoadThunk returns a function that when called will block waiting for a Timestamp.
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *SceneOHistoryLoader) LoadThunk(key int) func() ([]Timestamp, error) {
	l.mu.Lock()
...
func (l *SceneOHistoryLoader) LoadAll(keys []int) ([][]Timestamp, []error) {
	results := make([]func() ([]Timestamp, error), len(keys))
	...
	for i, thunk := range results {
		timestamps[i], errors[i] = thunk()
	}
	return timestamps, errors
}

// LoadAllThunk returns a function that when called will block waiting for a Timestamps.
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *SceneOHistoryLoader) LoadAllThunk(keys []int) func() ([][]Timestamp, []error) {
	results := make([]func() ([]Timestamp, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *SceneOHistoryLoader) Prime(key int, value []Timestamp) bool {
	l.mu.Lock()
	var found bool
	if _, found = l.cache[key]; !found {
		// make a copy when writing to the cache, its easy to pass a pointer in from a loop var
		// and end up with the whole cache pointing to the same value.
		cpy := make([]Timestamp, len(value))
		copy(cpy, value)
		l.unsafeSet(key, cpy)
	}
	l.mu.Unlock()
...
func (l *SceneOHistoryLoader) unsafeSet(key int, value []Timestamp) {
	if l.cache == nil {
		l.cache = map[int][]Timestamp{}
	}
	l.cache[key] = value
}
